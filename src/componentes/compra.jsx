...
export default function Compra() {
  ...
  const añadir = (e) => {
    ...
    setItems(actuales => [{ id: Date.now(), nombre: nombre.trim(), cantidad: cantidad.trim() || '1', categoria, comprado: false }, ...actuales]);
    setNombre('');
    setCantidad('1');
    setCategoria('Otros');
  ...
  const generarDesdeCalendario = () => {
    ...
    const nuevos = [];
    const vistos = new Set(items.map(i => i.nombre.toLowerCase()));
    ...
    comidas.filter(c => c.modo === 'receta').forEach(comida => {
      ...
      (receta?.ingredientes || []).forEach(ingrediente => {
        ...
        if (!vistos.has(clave)) {
          vistos.add(clave);
          nuevos.push({ id: `${Date.now()}-${nuevos.length}`, nombre: ingrediente, cantidad: '1', categoria: 'Despensa', comprado: false });
        }
      });
    });
  ...
  return (
    <div className="compra-app">
      <header className="compra-header">
        ...
        <button className="compra-icon-btn compra-add" onClick={() => setMostrarForm(true)} aria-label="Añadir producto"><Icono nombre="plus" /></button>
      </header>
      ...
      <div className="compra-lista">
        ...
        {CATEGORIAS.map(cat => {
          ...
          return <section className="compra-grupo" key={cat}><h2>{cat}</h2>{grupo.map(item => <Item key={item.id} item={item} alternar={alternar} eliminar={eliminar} />)}</section>;
        })}
        ...
        {comprados.length > 0 && (
          <section className="compra-grupo compra-comprados">
            ...
            {comprados.map(item => <Item key={item.id} item={item} alternar={alternar} eliminar={eliminar} />)}
          </section>
        )}
      </div>

      {mostrarForm && <><div className="compra-overlay" onClick={() => setMostrarForm(false)} /><div className="compra-sheet"><div className="compra-handle"/><h2>Añadir producto</h2><form onSubmit={añadir}><label>Producto<input autoFocus value={nombre} onChange={e => setNombre(e.target.value)} placeholder="Ej. Leche" /></label><div className="compra-form-row"><label>Cantidad<input value={cantidad} onChange={e => setCantidad(e.target.value)} /></label><label>Categoría<select value={categoria} onChange={e => setCategoria(e.target.value)}>{CATEGORIAS.map(c => <option key={c}>{c}</option>)}</select></label></div><button className="compra-save" disabled={!nombre.trim()}>Añadir a la lista</button></form></div></>}
    </div>
  );
}

function Item({ item, alternar, eliminar }) {
  return <div className={`compra-item ${item.comprado ? 'hecho' : ''}`}><button className="compra-check" onClick={() => alternar(item.id)} aria-label={item.comprado ? 'Marcar pendiente' : 'Marcar comprado'}>{item.comprado && <Icono nombre="check" size={15} />}</button><div className="compra-item-info"><strong>{item.nombre}</strong><span>{item.cantidad}</span></div><button className="compra-delete" onClick={() => eliminar(item.id)} aria-label="Eliminar"><Icono nombre="trash" size={17} /></button></div>;
}
